...
class Peer():
    ...
    def __init__(self, peer_id, peer_class, peer_pseudonym, local_data, labels, criterion,
...
#======================================= Start of training function ===========================================================#
    def participant_update(self, global_epoch, model, attack_type = 'no_attack', malicious_behavior_rate = 0,
        ...
        if (attack_type == 'label_flipping') and (self.peer_type == 'attacker'):
            ...
            if r <= malicious_behavior_rate:
                if dataset_name not in ['ADULT', 'IMDB']:
                    print(self.peer_id, ' attack ', source_class, ' to ', target_class, '!')
        ...
        model.train()
        # if global_epoch == 1 and self.peer_id == 2:
        #     iwant = model.state_dict()
        epochs_loss = []    
        x, y = None, None
        back_flag = 0
        for epoch in range(epochs):
            ...
            for batch_idx, (data, target) in enumerate(train_loader):
                ...
                if (attack_type == 'backdoor') and (self.peer_type == 'attacker')  and (np.random.random() <= malicious_behavior_rate):
                    if back_flag == 0:
                        print(self.peer_id, 'backdoor attack ！')
                        back_flag = 1
                    pdata = data.clone()
        ...
        if (attack_type == 'gaussian_noise' and self.peer_type == 'attacker'):
            print(self.peer_id, 'gaussian_noise attack ！')
    ...
    # byf
    def fdfl_choose_peers(self, honest_id, global_peers, frac_peers, epoch, use_reputation=False):
        #自己不能被排除
        self.local_reputation[self.peer_id] = max(self.local_reputation)
        if use_reputation and epoch >= 10 and self.peer_type != 'attacker':
            q1 = np.quantile(self.local_reputation, 0.2)
            idxs = np.where(self.local_reputation >= q1)
            peers = np.arange(self.num_peers)
            candidates = peers[idxs]
            m = min(int(frac_peers * self.num_peers), len(candidates))
            selected_peers = np.random.choice(candidates, m, replace=False)
        else:
            m = max(int(frac_peers * self.num_peers), 2)
            selected_peers = np.random.choice(range(self.num_peers), m, replace=False)

        if self.peer_id == honest_id:
            print('\n' + str(self.peer_id) + 'selected Peers:')
            for i, p in enumerate(selected_peers):
                print("Peer " + str(p) + ' is ', global_peers[p].peer_type,
                      'reputation:', self.local_reputation[p])
        return selected_peers

    def refuse_wights(self, honest_id, global_peers, epoch, get_index, use_reputation=False):
        dele = 0
        # 自己不能被排除
        self.local_reputation[self.peer_id] = max(self.local_reputation)
        if use_reputation and epoch >= 10 and self.peer_type != 'attacker':
            ...
            candidates = peers[idxs]
            needToDel = []
            for i, j in enumerate(get_index):
        ...
        if self.peer_id == honest_id:
            print('\n' + str(self.peer_id) + 'selected Peers after refuse(delete ' + str(dele) + '): ')
            for i, p in enumerate(get_index):
                print("Peer " + str(p) + ' is ', global_peers[p].peer_type,
                      'reputation:', self.local_reputation[p])
        return get_index
    ...
    def update_reputation(self, sim_dict, exchange_list):
        for k in exchange_list:
            self.update_local_reputation(k, sim_dict[k])
...
class FL:
    ...
    class_per_peer=2, samples_per_class= 250, rate_unbalance = 1, alpha = 1,source_class = None):
        ...
        #Loading of data
        self.trainset, self.testset, user_groups_train, tokenizer, attack_label = distribute_dataset(self.attackers_ratio, self.dataset_name, self.num_peers, self.num_classes,
        self.dd_type, self.class_per_peer, self.samples_per_class, self.alpha)

        #byf
        if self.dd_type == 'EXTREME_NON_IID':
            self.source_class = attack_label

        self.test_loader = DataLoader(self.testset, batch_size = self.test_batch_size,
            shuffle = False, num_workers = 0)

        if self.dataset_name == 'MNIST':
            self.test_loader = DataLoader(self.testset, sampler=ImbalancedDatasetSampler(self.testset),
        ...
        for p in peers:
            ...
            if m_ > 0 and contains_class(self.local_data[p], self.source_class):
                if attack_flag == 0:
                    self.attack_id = p
                    attack_flag = 1
                self.peers[p] = Peer(p, user_groups_train[p]['labels'], 'Peer ' + str(p),
...
#======================================= Start of testning function ===========================================================#
    def test(self, identity, model, device, test_loader, dataset_name = None):
        model.eval()
        test_loss = []
        correct = 0
        n = 0
        ...
        print(identity + ' Average test loss: {:.4f}, Test accuracy: {}/{} ({:.2f}%)\n'.format(test_loss, correct, n,
           100*correct / n))
        if identity[0] == 'h':
            self.yValue.append(100 * correct / n)
        else:
            self.attackValue.append(100 * correct / n)
...
#Test label prediction function    
    ...
    def run_experiment(self, attack_type = 'no_attack', malicious_behavior_rate = 0,
        ...
        for i in range(self.num_peers):
            fdfl_local_model[i] = copy.deepcopy(simulation_model)
        ...
        for epoch in tqdm_notebook(range(start_round, self.global_rounds)):
            ...
            #byf
            fdfl_send_weights, fdfl_send_masks, fdfl_get_index = {}, {}, {}
            fdfl_peers_trust, fdfl_local_weights = {}, {}
            for i in range(self.num_peers):
                fdfl_send_weights[i] = []
                fdfl_send_masks[i] = []
                fdfl_get_index[i] = []
                fdfl_peers_trust[i] = []
                fdfl_local_weights[i] = []
            ...
            for peer in range(self.num_peers):
                ...
                if rule == 'ffl':
                    ...
                    # if epoch == 0 :
                    #     self.peers[peer].weights_table =
                    # for key in ffl_local_weights[peer]:
                    #     ffl_local_weights[peer][key] *= -1
                elif rule == 'mkrum' or rule == 'foolsgold':
                    local_models.append(peer_local_model)
                elif rule == 'fdfl':
                    fdfl_local_weights[peer] = copy.deepcopy(peer_local_model).state_dict()
                else:
            ...
            #byf
                ...
                for peer in range(self.num_peers):
                    fdfl_selected_peers = self.peers[peer].fdfl_choose_peers(self.honest_id, self.peers, self.frac_peers, epoch, use_reputation=True)
                    peer_send_weight, peer_send_mask = fdfl_local_fragment_mast(fdfl_local_weights[peer])
                    fdfl_send_weights[peer] = peer_send_weight
                    fdfl_send_masks[peer] = peer_send_mask
                    for index in fdfl_selected_peers:
                        if index == peer:
                            continue
                        fdfl_get_index[index].append(peer)
                for peer in range(self.num_peers):
                    if peer == self.honest_id:
                        cur_time = time.time()
                    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
                    fdfl_get_index[peer] = self.peers[peer].refuse_wights(self.honest_id, self.peers, epoch, fdfl_get_index[peer], use_reputation=True)
                    fdfl_mixed_local_weights = {}
                    fdfl_mixed_local_weights[peer] = fdfl_local_weights[peer]
                    for index in fdfl_get_index[peer]:
                        fdfl_mixed_local_weight = {}
                        for key in fdfl_local_weights[peer].keys():
                            fdfl_local_weights[peer][key] = fdfl_local_weights[peer][key].to(device)
                            fdfl_send_masks[index][key] = fdfl_send_masks[index][key].to(device)
                            fdfl_send_weights[index][key] = fdfl_send_weights[index][key].to(device)
                            fdfl_mixed_local_weight[key] = fdfl_local_weights[peer][key] * (1 - fdfl_send_masks[index][key]) + fdfl_send_weights[index][key]
                        fdfl_mixed_local_weights[index] = fdfl_mixed_local_weight
                    sim_dict, score = score_mixed_updates(fdfl_mixed_local_weights, copy.deepcopy(fdfl_local_model[peer]))
                    self.peers[peer].update_reputation(score, fdfl_get_index[peer])
                    models = []
                    idxs = []
                    for k, s in sim_dict.items():
                        models.append(fdfl_mixed_local_weights[k])
                        idxs.append(k)
                    if epoch < 1:
                        fdfl_local_weights[peer] = simple_median(models)
                    else:
                        q1 = np.quantile(self.peers[peer].local_reputation, 0.2)
                        v = self.peers[peer].local_reputation - q1
                        v = np.tanh(v)
                        v[v < 0] = 0
                        fdfl_peers_trust[peer].append(v)
                        v = v[idxs]
                        #print(v)
                        fdfl_local_weights[peer] = fdfl_average_weights(models, sim_dict, v)
                    if peer == self.honest_id:
                        t = time.time() - cur_time
                        print(str(self.honest_id) + ' Aggregation took', np.round(t, 4))
                        cpu_runtimes.append(t)
            else:
                global_weights = average_weights(local_weights, [1 for i in range(len(local_weights))])
                ##############################################################################################
            ...
            for peer in range(self.num_peers):
                g_model = copy.deepcopy(fdfl_local_model[peer])
                fdfl_local_model[peer].load_state_dict(fdfl_local_weights[peer])
            if epoch >= self.global_rounds-10:
                last10_updates.append(fdfl_local_weights[self.honest_id])
            fdfl_local_model[self.honest_id] = fdfl_local_model[self.honest_id].to(device)
            if self.num_attackers != 0:
                fdfl_local_model[self.attack_id] = fdfl_local_model[self.attack_id].to(device)
                self.test('attack ' + str(self.attack_id), fdfl_local_model[self.attack_id], self.device,
                          test_loader=self.test_loader, dataset_name=self.dataset_name)
            current_accuracy, test_loss = self.test('honest ' + str(self.honest_id), fdfl_local_model[self.honest_id], self.device,
                                                    test_loader=self.test_loader, dataset_name=self.dataset_name)
            ...
            #byf
            actuals, predictions = self.test_label_predictions(fdfl_local_model[self.honest_id], self.device, self.test_loader, dataset_name=self.dataset_name)
            ...
            if attack_type == 'backdoor':
                ...
                backdoor_asr = self.test_backdoor(fdfl_local_model[self.honest_id], self.device, self.test_loader,
                                backdoor_pattern, source_class, target_class)
            print('\nBackdoor ASR', backdoor_asr)
            
            state = {
                'epoch': epoch,
                'state_dict': fdfl_local_model[self.honest_id].state_dict(),
            ...
            if epoch == self.global_rounds-1:
                ...
                #byf
                fdfl_local_model[self.honest_id].load_state_dict(global_weights)
                current_accuracy, test_loss = self.test('honest ' + str(self.honest_id), fdfl_local_model[self.honest_id], self.device,
                                                        test_loader=self.test_loader, dataset_name=self.dataset_name)
        ...
        print('honest!')
        plt.plot(self.yValue, 'b' if attack_type == 'no_attack' else 'r')
        plt.axis([0, self.global_rounds, 0, 100])
        plt.xlabel(xlabel='epochs', fontsize=15)
        plt.ylabel(ylabel='accuracy', fontsize=15)
        plt.show()
        print('attacker!')
        plt.plot(self.attackValue, 'b' if attack_type == 'no_attack' else 'r')
        plt.axis([0, self.global_rounds, 0, 100])
        plt.xlabel(xlabel='epochs', fontsize=15)
        plt.ylabel(ylabel='accuracy', fontsize=15)
        plt.show()
