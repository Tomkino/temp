...
def score_mixed_updates(mixed_updates, gmodel):
    local_models = []
    ...
    for i, k in enumerate(peers):
        ...
        score[k] = sim_score[i].item()
    return sim_dict, score


def score_all(all_grads):
    n_grads = torch.norm(all_grads, dim = -1)
    med = torch.median(n_grads)
    baseline = n_grads[0]
    maxD = abs(med - baseline)
    for i in range(len(n_grads)):
        t = abs(med - n_grads[i])
        if t > maxD:
            maxD = t
            baseline = n_grads[i]
    d = abs(baseline - n_grads)
    #med = torch.mean(n_grads)
    return d

def score_last(all_grads, sl):
    n = len(all_grads)
    last = torch.empty([n, sl])
    for i in range(n):
        last[i] = all_grads[i][-sl:]

    # med = torch.mean(last, dim = 0)[0]
    # med = med.unsqueeze(0)
    # baseline = last[0][0]
    # cs = F.cosine_similarity(med, last)
    # minC = cs[0]
    ...
    # cs = F.cosine_similarity(baseline, last)
    centroid = torch.max(last, dim=0)[0]
    centroid = centroid.unsqueeze(0)
    cs = F.cosine_similarity(centroid, last)
    return 1 - cs
#################################################################################################################
...
def median(updates):
    ...
    for key in med.keys():
        shape = [len(updates)]
        shape+= list(med[key].size())
        layer = torch.empty(shape)
        for i, model in enumerate(updates):
            layer[i] = model[key]
        
        layer_med = torch.median(layer,  dim=0, keepdim = True)[0]
        med[key] = layer_med[0]
...
# Get average weights
def fdfl_average_weights(w, marks, local_reputation):
    marks = list(marks.values())
    if sum(marks) == 0:
        marks = marks + 1
    local_reputation = local_reputation / sum(local_reputation)
    tTrust = []
    for i in range(len(marks)):
        tTrust = append(tTrust, marks[i] * 0.6 + local_reputation[i] * 0.4)
    tTrust = tTrust / sum(tTrust)
    """
    ...
    for key in w_avg.keys():
        # w_avg[key].to(device)
        #w_avg[key] = w_avg[key] * marks[0] * 0.5 + w_avg[key] * local_reputation[0] * 0.5
        w_avg[key] = w_avg[key] * tTrust[0]
    for key in w_avg.keys():
        for i in range(1, len(w)):
            w_avg[key] = w_avg[key].to(device)
            w[i][key] = w[i][key].to(device)
            # print(w[i][key].is_cuda)
            # print(w_avg.is_cuda)
            #w_avg[key] += w[i][key] * marks[i] * 0.5 + w[i][key] * local_reputation[i] * 0.5
            w_avg[key] += w[i][key] * tTrust[i]
...
def Krum(updates, f, multi = False):
    update = torch.nn.utils.parameters_to_vector(updates[0].parameters())
    n = len(updates)
    updates_ = torch.empty([n, len(update)])
    updates_[0] = update
    for i in range(1, n):
      updates_[i] = torch.nn.utils.parameters_to_vector(updates[i].parameters())
    k = n - f - 2
