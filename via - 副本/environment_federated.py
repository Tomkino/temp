...
class FL:
    ...
    class_per_peer=2, samples_per_class= 250, rate_unbalance = 1, alpha = 1,source_class = None):
        ...
        #Loading of data
        self.trainset, self.testset, user_groups_train, tokenizer, attack_label = distribute_dataset(self.source_class, self.dataset_name, self.num_peers, self.num_classes,
        ...
        for p in peers:
            if m_ > 0 and contains_class(self.local_data[p], self.source_class):
                ...
                self.peers[p].labels = user_groups_train[p]['labels']
                m_-= 1
            else:
                self.peers[p] = Peer(p, user_groups_train[p]['labels'], 'Peer ' + str(p),
                self.local_data[p], self.labels[p],
        ...
        for epoch in tqdm_notebook(range(start_round, self.global_rounds)):
            ...
            for peer in selected_peers:
                ...
                if rule == 'ffl':
                    ...
                    # if (attack_type == 'sign_flipping') and (self.peers[peer].peer_type == 'attacker'):
            ...
            if rule == 'fedavg':
                ...
                cur_time = time.time()
                sim_dict, exchange_list = score_mixed_updates(mixed_updates, copy.deepcopy(simulation_model), exchange_list)
            ...
            current_accuracy, test_loss = self.test(simulation_model, self.device,
